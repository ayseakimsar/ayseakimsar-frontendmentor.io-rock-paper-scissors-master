const allButtons = document.querySelectorAll(".element_btn");
...
const gameResultText = document.querySelector(".game_result_text");
...
const winner = function () {
  const difference = yourPick - housePick;
  if (difference % 2 == 0) {
    if (yourPick > housePick) {
      return `You won!`;
    } else if (housePick > yourPick) {
...
const disableButtons = function (value) {
  allButtons.forEach(function (b) {
    b.disabled = value;
  });
};

const displayIcon = function (pickedValue) {
  if (pickedValue == 0) {
    pickedIcon =
...
const calcDisplayScore = function () {
  if (result == "You won!") {
    score++;
...
const elementButtonClicked = function (btn) {
  allButtons.forEach(function (b) {
    if (b.getAttribute("value") == btn) btn = b;
    else return;
  });
  clicked = btn;

  housePick = Math.ceil(Math.random() * 5) - 1;
  yourPick = btn.getAttribute("value");
  result = winner();

  // get the name of what house picked
  allButtons.forEach(function (btn) {
    if (btn.getAttribute("value") == housePick) {
      housePickName = btn.getAttribute("name");
    } else return;
  });

  //1. hide pentagon
  pentagon.classList.add("hidden");
  // 2. make visible step 2 and disable buttons
  disableButtons(true);
  step2.classList.remove("hidden");
  // 3. add the class of the chosen button next to user_picked
  userPicked.classList.add(`btn_${btn.getAttribute("name")}`);
  // 4. add the svg element inside the container
  userPicked.innerHTML = displayIcon(yourPick);
  // 5. add the class of the button next to house_picked and add the svg element inside the house_pick container
  setTimeout(function () {
    housePicked.innerHTML = displayIcon(housePick);
    housePicked.classList.add(`btn_${housePickName}`);
  }, 1000);
  // 6. display game result
  setTimeout(() => {
    gameResult.classList.remove("hidden");
    gameResultText.innerHTML = result;
    // 7. calculate and display score
    calcDisplayScore();
  }, 2000);
};
...
const playAgain = function () {
  ...
  userPicked.classList.remove(`btn_${clicked.getAttribute("name")}`);
  userPicked.innerHTML = "";
  housePicked.classList.remove(`btn_${housePickName}`);
